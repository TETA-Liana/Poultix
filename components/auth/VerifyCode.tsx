...
type FarmOverviewRouteProp = RouteProp<RootStackParamList, 'VerifyCode'>;

export default function VerifyCode() {
    const route = useRoute<FarmOverviewRouteProp>();
    ...
    const handleConfirm = async () => {
        ...
        if (enteredCode.length === 4) {
            ...
            try {
                const response = await axios.post(hostConfig.host + '/verifyCode', {
                    email,
                    code: enteredCode,
                });
                Animated.sequence([
                            ...
                            {keypadData.map((item, index) => (
                                ...
                                >
                                    <View style={tw`items-center`}>
                                        ...
                                        {item.number ? (
                                            <Text style={tw`text-xs text-gray-500`}>{item.number}</Text>
                                        ) : null}
                                    </View>
                                </TouchableOpacity>
                            ))}
